...
test:
	@echo "→ Running tests..."
	@cd $(SRC_DIR) && go test -timeout=$(TEST_TIMEOUT) ./...
...
test-short:
	...
	@cd $(SRC_DIR) && go test -short -timeout=$(TEST_TIMEOUT) ./...
	@echo "✓ Short tests passed"

test-coverage:
	@echo "→ Running tests with coverage analysis..."
	@cd $(SRC_DIR) && go test -timeout=$(TEST_TIMEOUT) -coverprofile=../$(COVERAGE_DIR)/coverage.out -covermode=count ./...
